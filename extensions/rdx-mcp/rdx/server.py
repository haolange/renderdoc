...
def _tool_catalog_path() -> Path:
    return Path(__file__).resolve().parent / "spec" / "tool_catalog_196.json"
...
def _create_mcp() -> FastMCP:
    kwargs: Dict[str, Any] = {}
    description = "RenderDoc MCP tools (196 doc tools + 3 extensions)"
    try:
...
async def _dispatch_core(action: str, args: Dict[str, Any]) -> str:
    ...
    if action == "healthcheck":
        ...
        artifact_dir = Path(_runtime.config.get("artifact_dir", "./rdx_artifacts"))
        try:
...
async def _dispatch_shader(action: str, args: Dict[str, Any]) -> str:
    ...
    if action in {"get_debug_state", "list_replacements", "revert_replacement", "edit_and_replace", "get_messages", "compile", "save_binary", "extract_binary", "get_source", "list_entry_points", "get_bindpoint_mapping", "get_constant_block_layout", "get_constant_buffer_contents", "get_reflection", "get_disassembly"}:
        pass
    else:
    ...
    if action in {"list_entry_points", "get_bindpoint_mapping", "get_constant_block_layout", "get_constant_buffer_contents", "get_reflection", "get_disassembly"}:
...
async def _dispatch_macro(action: str, args: Dict[str, Any]) -> str:
    ...
    if action == "find_pass_by_marker":
        ...
        query = {"name_regex": str(args["name_regex"])}
        return await _dispatch_event("search_actions", {"session_id": session_id, "query": query, "max_results": args.get("max_results", 20)})
    ...
    if action == "shader_hotfix_validate":
        ...
        if not repl_payload.get("success"):
            return repl_resp
        screenshot_before = await _dispatch_export("screenshot", {"session_id": session_id, "output_path": str(Path(tempfile.gettempdir()) / "before_hotfix.png")})
        screenshot_after = await _dispatch_export("screenshot", {"session_id": session_id, "output_path": str(Path(tempfile.gettempdir()) / "after_hotfix.png")})
