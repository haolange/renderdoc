...
from rdx.vfs.provider import VfsProvider
...
def _rows_to_tsv(rows: list[Dict[str, Any]]) -> str:
    if not rows:
        return to_tsv_string([], columns=["name", "kind"], include_header=True)
    preferred = ["event_id", "resource_id", "texture_id", "stage", "name", "kind"]
    keys = set()
    for row in rows:
        keys.update(row.keys())
    ordered = [k for k in preferred if k in keys]
    for key in sorted(keys):
        if key not in ordered and key not in {"details_json_path", "details_json_url"}:
            ordered.append(key)
    return to_tsv_string(rows, columns=ordered, include_header=True)
...
async def _cmd_capture_open(args: argparse.Namespace) -> int:
    ...
    payload = canonical_success(
        ...
        data={
            ...
            "state_path": "temp://rdx_cli/session_state.json",
...
async def _cmd_vfs_ls(args: argparse.Namespace) -> int:
    ...
    payload = await provider.ls(args.path, session_id=session_id, execute=_exec)
    rows = payload.get("children", [])
    if args.format == "json":
        _print_json(canonical_success(result_kind="rdx.vfs.ls", data=payload, transport="cli"))
    else:
        print(_rows_to_tsv(rows if isinstance(rows, list) else []))
    return EXIT_OK


async def _cmd_vfs_cat(args: argparse.Namespace) -> int:
    session_id = _default_session_id(args.session_id)
    provider = VfsProvider()
    ...
    payload = await provider.cat(args.path, session_id=session_id, execute=_exec)
    if args.format == "json":
        _print_json(canonical_success(result_kind="rdx.vfs.cat", data={"path": args.path, "content": payload}, transport="cli"))
    else:
...
async def _cmd_vfs_get(args: argparse.Namespace) -> int:
    session_id = _default_session_id(args.session_id)
    provider = VfsProvider()
    ...
    payload = await provider.get(args.path, out_path=args.out, session_id=session_id, execute=_exec)
    _print_json(canonical_success(result_kind="rdx.vfs.get", data=payload, transport="cli"))
    return EXIT_OK
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_capture_open.add_argument("--artifact-dir", default=str(Path("rdx_artifacts").resolve()))
    ...
    p_vfs = sub.add_parser("vfs", help="Virtual filesystem")
    s_vfs = p_vfs.add_subparsers(dest="vfs_cmd", required=True)
    p_vfs_ls = s_vfs.add_parser("ls")
    p_vfs_ls.add_argument("path", nargs="?", default="/")
    p_vfs_ls.add_argument("--session-id", default=None)
    p_vfs_ls.add_argument("--format", choices=["json", "tsv"], default="tsv")
    p_vfs_ls.add_argument("--connect", action="store_true")
    p_vfs_cat = s_vfs.add_parser("cat")
    p_vfs_cat.add_argument("path")
    p_vfs_cat.add_argument("--session-id", default=None)
    p_vfs_cat.add_argument("--format", choices=["json", "text"], default="text")
    p_vfs_cat.add_argument("--connect", action="store_true")
    p_vfs_get = s_vfs.add_parser("get")
    p_vfs_get.add_argument("path")
    p_vfs_get.add_argument("--out", required=True)
    p_vfs_get.add_argument("--session-id", default=None)
    p_vfs_get.add_argument("--connect", action="store_true")
...
async def _main_async(args: argparse.Namespace) -> int:
    ...
    if args.command == "vfs":
        if args.vfs_cmd == "ls":
            return await _cmd_vfs_ls(args)
        if args.vfs_cmd == "cat":
            return await _cmd_vfs_cat(args)
        if args.vfs_cmd == "get":
            return await _cmd_vfs_get(args)
